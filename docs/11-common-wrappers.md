...
每个输入上限 512 MiB；每个根任务累计 8 GiB、10000 文件、16 个包、4 层。ZIP 中央目录和 7z 常驻元数据分配限制 64 MiB；7z 辅助解码分配限制 256 MiB。固实块写入受目录锁保护的临时磁盘映射，单块和累计展开不得超过 8 GiB，离开当前块后清理；不分配与整块输出等大的堆缓冲。缓存与最终输出可能同时占用磁盘。
