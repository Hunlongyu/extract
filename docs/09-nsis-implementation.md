...
Solid 模式将头与文件块共同压缩。为生成准确清单，`--list` 也要展开这些块。实现使用 `%TEMP%` 下随机、独立、受目录锁保护的缓存，最多允许 8 GiB 数据加 64 MiB 元数据和 4 字节长度字段；通过保留原始读写句柄阻止其它写入或删除，只建立只读映射。无论正常结束还是解析失败，都按本任务创建清单清理缓存，不递归遍历未知目录。
...
当前限制：输入 512 MiB，文件 10,000 个，最终文件总输出 8 GiB；NSIS 元数据 64 MiB、指令 100,000 条、语言表 256 个、字符串引用深度 16，字符串解析工作量另设上限，LZMA 字典 256 MiB。PE 外壳原始节末尾限制 16 MiB，在其后 64 KiB 的对齐区间定位本层 NSIS。
