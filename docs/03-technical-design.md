...
工作进程与取消的当前实现已落地，具体协议、Job Object、退出处理、临时清理及中断恢复边界见 [工作进程设计](22-worker-cancellation.md)。该部分为 v0.7.0 发布后的本地开发改动；下文各版本段落保留历史背景。

日期：2026-09-07。0.5.0 已实现 MSI 多媒体、标准 CAB、Burn 首批结构、选定 Inno/NSIS 结构、ZIP/7z、递归展开、统一输出和系统通知；v0.7.0 发布后补充工作进程和可选超时。最新模块与边界见各实现文档及 README。

## 1. 结构

```text
Extract.exe：Win32 参数入口 / 任务控制 / 系统通知
    └─ 相同 EXE 的内部 worker 顺序处理输入包
        └─ extract_core：本项目 C++20 核心
            ├─ MSI / Inno / NSIS / Burn 安装包处理器
            ├─ Velopack / Squirrel 完整离线包
            ├─ MSIX / APPX / Bundle 清单与块校验
            ├─ 通用归档与压缩基础库
            └─ 有边界的输入流 / 文件计划 / 校验 / 安全写入
```

安装包解析逻辑编入自己的核心库。当前本地实现使用同一个 EXE 的 worker 顺序处理每个输入包、隔离解析崩溃并支持可选超时，不调用第三方解包程序，不建立通用插件 ABI、常驻服务或数据库。
...
已完成首批本地实现。支持的清单版本、块映射、Bundle 架构/资源关系、目录布局及验证边界见 [MSIX/APPX 实现](25-msix-appx.md)。
...
本地已实现两种独立布局：Velopack 固定标记定位有界 ZIP；Squirrel.Windows 定位 DATA/131 资源，再校验 RELEASES 并把 full.nupkg 流式解码到受控磁盘缓存。共用本项目 ZIP 校验器和输出层，解析 nuspec、整理 lib 目录；不运行更新器和安装钩子。差分包缺少精确基线时明确拒绝。源码锚点、样本和边界见 [实现记录](24-update-packages.md)。
...
嵌套处理由 `core/tree.cpp` 实现：最多 4 层、32 个包（均含根层）。取消累计 8 GiB / 10000 文件的固定预算，统计使用有溢出校验的整数，每层落盘前检查其实际预计大小与可用磁盘空间。原始内层包计入统计，按 SHA-256 检查活动链；MSI/Burn 因外置来源可能不同，不复用只按输入包摘要缓存的结果。每层提交后释放输入和 Solid 缓存，再处理子包。打开子包后复核摘要，防止层间替换。所有原始内层包保留，报告保存父子关系、累计输出和子状态；内层失败使祖先报告 partial，不删除已经成功提交的外层。已实现可选单包超时，整进程内存硬限制仍未设置；大文件读取及各解码器剩余约束见 [大安装包处理](17-large-packages.md)。
